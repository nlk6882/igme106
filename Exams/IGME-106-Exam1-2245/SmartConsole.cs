namespace IGME_106_Exam1_2245
{
    ...
    internal class SmartConsole
    {
        ...
        /// <summary>
        /// Helper method to prompt the user to enter a boolean.
        /// </summary>
        /// <param name="prompt">The string to use in the initial prompt</param>
        /// <returns>The final, valid, user-entered value.</returns>
        public static bool Prompt(string prompt, bool allowYN)
        {
            bool result = false;
            bool validResponse = false;
            while (!validResponse)
            {
                string response = Prompt(prompt).ToUpper();
                switch (response)
                {
                    case "TRUE":
                    case "T":
