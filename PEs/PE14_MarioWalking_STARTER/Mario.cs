...
namespace PE_MarioWalking
{
    ...
    class Mario
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            if(State == MarioState.FaceRight)
            {
                frame = 0;
                DrawWalking(SpriteEffects.None, spriteBatch);
            }
            if (State == MarioState.FaceLeft)
            {
                frame = 0;
                DrawWalking(SpriteEffects.FlipHorizontally, spriteBatch);
            }
            if (State == MarioState.WalkRight)
            {
                DrawWalking(SpriteEffects.None, spriteBatch);
                X = X + 1;
            }
            if (State == MarioState.WalkLeft)
            {
                DrawWalking(SpriteEffects.FlipHorizontally, spriteBatch);
                X = X + -1;

            }
            if (State == MarioState.CrouchRight)
            {
                frame = 6;
                DrawWalking(SpriteEffects.None, spriteBatch);
            }
            if (State == MarioState.CrouchLeft)
            {
                frame = 6;
                DrawWalking(SpriteEffects.FlipHorizontally, spriteBatch);
            }
        ...
        /// and or vertically
        /// </param>
        internal void DrawStanding(SpriteEffects flipSprite, SpriteBatch spriteBatch)
        {
            spriteBatch.Draw(
